    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        const auto renderScene = [&](Shader& shader)
        {
            ...
            if (modeFlag == 2)
            {
                ...
                {
                    modeChange = true;
                    particleVelocityFiled.renewParticle();
                    particleImplicit.renewParticle();
                    particle.renewParticle();
                }

                // get the current position of the particle
                glm::vec3 particlePositionVelocityFiled = particleVelocityFiled.updateVelocityField();
                glm::vec3 particlePositionImplicit = particleImplicit.updateConcenForceFieldImplicit();
                glm::vec3 particlePosition = particle.updateConcenForceField();

                drawCircle(0, 0, 0, 0.1);
                drawCircle(0, 0, 0, 0.3);
                drawCircle(0, 0, 0, 0.5);
                drawCircle(0, 0, 0, 0.7);
                drawCircle(0, 0, 0, 0.9);
                drawCircle(0, 0, 0, 1.1);
                drawCircle(0, 0, 0, 1.3);
                drawCircle(0, 0, 0, 1.5);
                drawCircle(0, 0, 0, 1.7);
                drawCircle(0, 0, 0, 1.9);
                drawCircle(0, 0, 0, 2.1);

                glm::mat4 model = glm::mat4(1.0f);
                model = glm::translate(model, particlePosition);
                shader.setMat4("model", model);
                /*std::cout << particle.updateConcenForceField().x << std::endl;*/
                renderSphere();

                model = glm::mat4(1.0f);
                model = glm::translate(model, particlePositionImplicit);
                shader.setMat4("model", model);
                /*std::cout << particle.updateConcenForceField().x << std::endl;*/
                renderSphere();

                model = glm::mat4(1.0f);
                model = glm::translate(model, particlePositionVelocityFiled);
                shader.setMat4("model", model);
                /*std::cout << particle.updateConcenForceField().x << std::endl;*/
                renderSphere();
            }
